...
// 🟢 Middleware xác thực JWT (có hỗ trợ token query)
exports.protect = async (req, res, next) => {
  try {
    let token;

    // 1️⃣ Ưu tiên: Lấy token từ query ?token=...
    if (req.query.token) {
      token = req.query.token;
      console.log("🔑 Token từ query:", token);
    }

    // 2️⃣ Nếu không có thì lấy token từ header Authorization: Bearer ...
    if (
      !token &&
...
// 🧑‍🏫 Middleware admin hoặc giáo viên
