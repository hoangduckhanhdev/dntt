...
exports.getQuestions = async (req, res) => {
  try {
    ...
    if (!tagList.length && typeof tag === "string" && tag.trim()) {
      tagList = [tag.trim()];
    }
...
exports.getQuestionFilters = async (req, res) => {
  try {
    ...
    if (!course) {
      return res.status(400).json({ message: "Thiếu course trên query string." });
    }
    if (!mongoose.isValidObjectId(course)) {
      return res.status(400).json({ message: "course không hợp lệ." });
    }

    const courseDoc = await Course.findById(course).select("sections").lean();
    if (!courseDoc) {
    ...
    const chapters = Array.from(
      new Set(
        (courseDoc.sections || [])
          .map((s) => (s?.title || "").toString().trim())
          .filter(Boolean)
      )
    );

    const qFilter = { course };
    if (typeof chapter === "string" && chapter.trim()) {
      qFilter.chapter = chapter.trim();
    }

    const docs = await ExamQuestionBank.find(qFilter).select("tags").lean();
    const tagSet = new Set();
    docs.forEach((d) => (d.tags || []).forEach((t) => t && tagSet.add(String(t).trim())));
...
exports.createQuestion = async (req, res) => {
  try {
    ...
    if (!b.course || !b.content || !b.type) {
      return res.status(400).json({ message: "Thiếu course, content hoặc type." });
    }
    ...
    if (!b.chapter || !String(b.chapter).trim()) {
      return res.status(400).json({ message: "Vui lòng chọn Chương." });
    }
    ...
    let score = typeof b.score === "number" && b.score > 0 ? b.score : 1;
...
exports.updateQuestion = async (req, res) => {
  try {
    ...
    let newCorrectAnswer =
      typeof b.correctAnswer !== "undefined" ? b.correctAnswer : question.correctAnswer;
    let newExpectedAnswer =
      typeof b.expectedAnswer !== "undefined" ? b.expectedAnswer : question.expectedAnswer;
    let newAiAutoGrade =
      typeof b.aiAutoGrade !== "undefined" ? !!b.aiAutoGrade : !!question.aiAutoGrade;
    let newScore =
      typeof b.score === "number" && b.score > 0 ? b.score : question.score || 1;
    ...
    if (newType === "multiple_choice") {
      ...
      const v = (newCorrectAnswer || "").toString().trim().toLowerCase();
      if (v !== "true" && v !== "false") {
        return res.status(400).json({
          message: 'Câu hỏi Đúng/Sai phải có correctAnswer là "true" hoặc "false".',
        });
    ...
    // ✅ chapter update có trim (tránh null/space)
    if (typeof b.chapter !== "undefined") {
      question.chapter = String(b.chapter || "").trim();
    }
