...
exports.getAllCourses = async (req, res) => {
  try {
    ...
    // ✅ QnA stats
    const qnaStatsMap = new Map();
    if (courseIds.length) {
      const qnaList = await Question.find({ course: { $in: courseIds } })
        .select("course answer")
        .lean();

      qnaList.forEach((q) => {
        const cid = String(q.course);
        if (!qnaStatsMap.has(cid)) qnaStatsMap.set(cid, { total: 0, unanswered: 0 });
        const row = qnaStatsMap.get(cid);
        row.total += 1;
        if (!q.answer || !q.answer.trim()) row.unanswered += 1;
      });
    }

    // ✅ Đếm học viên đăng ký THÀNH CÔNG theo RegisterCourse.paymentStatus = "paid"
    const studentCountMap = new Map();
    if (courseIds.length) {
      const counts = await RegisterCourse.aggregate([
        { $match: { courseId: { $in: courseIds }, paymentStatus: "paid" } },
        { $group: { _id: "$courseId", count: { $sum: 1 } } },
      ]);

      counts.forEach((it) => studentCountMap.set(String(it._id), it.count));
    }
    ...
    const formatted = courses.map((course) => {
      ...
      return {
        ...
        // ✅ dùng số đếm thật theo đăng ký paid
        studentsCount: studentCountMap.get(cid) || 0,
