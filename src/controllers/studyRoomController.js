...
async function canManageRoom(req, room) {
  ...
  if (["teacher", "giangvien", "instructor"].includes(role)) {
    const isCreator =
...
exports.getMyRooms = async (req, res, next) => {
  try {
    ...
    const role = req.user?.role;
    const orConditions = [{ "members.user": userId }, { isPublic: true }];
...
exports.createRoom = async (req, res, next) => {
  try {
    ...
    const { name, courseId, lessonName, isPublic } = req.body;
    ...
    const created = await StudyRoom.create({
      name: name.trim(),
      course: safeCourseId,
    ...
    const room = await StudyRoom.findById(created._id)
      .populate("course", "title")
      .populate("createdBy", "name");
...
exports.updateRoom = async (req, res, next) => {
  try {
    ...
    if (!room) {
      return res.status(404).json({ message: "Không tìm thấy phòng học nhóm." });
    }
    ...
    const { name, lessonName, isPublic, isArchived } = req.body;

    if (name && name.trim()) room.name = name.trim();
    if (typeof lessonName === "string") room.lessonName = lessonName.trim();
    if (typeof isPublic === "boolean") room.isPublic = isPublic;
    if (typeof isArchived === "boolean") room.isArchived = isArchived;
