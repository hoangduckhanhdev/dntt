...
export default function AdminTeachers() {
  ...
  const handleAdd = async () => {
    const { value: formValues } = await Swal.fire({
      ...
      preConfirm: () => ({
        ...
        expertise: document.getElementById("swal-expertise").value,
        phone: document.getElementById("swal-phone").value,
        zaloLink: document.getElementById("swal-zalo").value,
        messengerLink: document.getElementById("swal-messenger").value,
        registerLink: document.getElementById("swal-register").value,
    ...
    if (!formValues.name?.trim()) {
      Swal.fire("Thiếu thông tin", "Tên giáo viên là bắt buộc.", "warning");
      return;
    }
    ...
    try {
      ...
      Swal.fire("Thành công!", "Giáo viên mới đã được tạo.", "success");
      fetchTeachers();
    } catch (err) {
      console.error("❌ createTeacher error:", err);
      Swal.fire("Lỗi!", "Không thể tạo giáo viên mới!", "error");
    }
  };
  ...
  const handleEdit = async (teacher) => {
      ...
      html: `
        <div style="display:flex; gap:16px; align-items:flex-start; max-height:70vh; overflow:auto; padding-right:4px;">
          <div style="flex:1 1 260px;">
            ...
            <input id="swal-email" type="email" class="swal2-input" value="${
              teacher.email || ""
            }" placeholder="Email nếu muốn đồng bộ User">

            <label style="font-size:12px; color:#6b7280; display:block; margin-top:4px;">Chức danh</label>
            <input id="swal-title" class="swal2-input" value="${
              teacher.title || ""
            }" placeholder="VD: Giảng viên Frontend">

            <label style="font-size:12px; color:#6b7280; display:block; margin-top:4px;">Chuyên môn</label>
            <input id="swal-expertise" class="swal2-input" value="${
              teacher.expertise || ""
            }" placeholder="VD: ReactJS, NodeJS">
            ...
            <input id="swal-zalo" class="swal2-input" value="${
              teacher.zaloLink || ""
            }" placeholder="https://zalo.me/...">

            <label style="font-size:12px; color:#6b7280; display:block; margin-top:4px;">Link Messenger</label>
            <input id="swal-messenger" class="swal2-input" value="${
              teacher.messengerLink || ""
            }" placeholder="https://m.me/...">

            <label style="font-size:12px; color:#6b7280; display:block; margin-top:4px;">Link đăng ký khóa học</label>
            <input id="swal-register" class="swal2-input" value="${
              teacher.registerLink || ""
            }" placeholder="Đường dẫn tới trang đăng ký">
    ...
    try {
      ...
      await updateTeacher(teacher._id, fd);
      Swal.fire("Thành công!", "Thông tin giáo viên đã được cập nhật.", "success");
      fetchTeachers();
    } catch (err) {
    ...
    <div className="bg-white p-6 rounded-xl shadow-md">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* pagination */}
          <div className="mt-4 flex flex-wrap items-center justify-between text-sm text-gray-600 gap-2">
            <span>
              Hiển thị{" "}
              <strong>
                {startIndex + 1}-
              ...
              / <strong>{teachers.length}</strong> giáo viên
            </span>
