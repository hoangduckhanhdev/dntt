...
import adminApi from "../../api/adminApi"; // ✅ dùng adminApi để có Authorization
...
export default function AdminCourseCurriculum() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // ✅ GET /api/admin/courses/:id (adminApi baseURL đã là /api/admin)
        const { data } = await adminApi.get(`/courses/${id}`);
        ...
        const normalized = rawSections.map((sec, sIdx) => ({
          ...
          lessons: Array.isArray(sec?.lessons)
            ? sec.lessons.map((ls, lIdx) => ({
                ...
                // ✅ mới: type + textContent
                type: ls?.type === "text" ? "text" : "video",
        ...
        console.error(e);
        alert("Không tải được khoá học! (Có thể thiếu token hoặc không đúng quyền)");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const addSection = () =>
    setSections((s) => [...s, { title: "Chương mới", order: s.length, lessons: [] }]);
  ...
  const removeLesson = (sIdx, lIdx) =>
    setSections((s) =>
      s.map((sec, i) =>
        i === sIdx ? { ...sec, lessons: (sec.lessons || []).filter((_, j) => j !== lIdx) } : sec
      )
    );
      ...
      s.map((sec, i) =>
        i === sIdx
          ? {
              ...
              lessons: (sec.lessons || []).map((ls, j) => {
                if (j !== lIdx) return ls;
                if ((ls.type || "video") !== "video") return ls;
                return { ...ls, video: toCleanWatchUrl(ls.video) };
              }),
            }
          : sec
      )
    ...
    try {
      ...
      const cleanedSections = sections.map((sec, sIdx) => ({
          ...
          lessons: sec.lessons.map((ls, lIdx) => {
            const isText = ls.type === "text";

            return {
              title: ls.title || `Bài ${lIdx + 1}`,
              duration: ls.duration || "",
              order: lIdx,
              type: isText ? "text" : "video",

              // 🔥 CHỐT
              video: isText ? "" : toCleanWatchUrl(ls.video || ""),
              textContent: isText ? (ls.textContent || "") : "",
            };
          }),
        }));
      ...
      await adminApi.put(`/courses/${id}/curriculum`, {
      ...
      console.error(e);
      alert("Lưu thất bại! (Có thể thiếu token hoặc không đúng quyền)");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-6 bg-orange-50 min-h-screen">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3 py-2 rounded-lg border ${
                dirty ? "border-rose-200 text-rose-600 hover:bg-rose-50" : "border-gray-200 text-gray-400 cursor-not-allowed"
              }`}
            >
              Huỷ thay đổi
            </button>
        ...
        <h1 className="text-2xl font-bold text-orange-600 mb-2">Chỉnh outline: {title}</h1>

        <div className="bg-white border border-orange-100 rounded-xl p-4 mb-6">
          <label className="block text-sm font-medium text-gray-700 mb-1">Link video demo (YouTube)</label>
          <input
            value={videoDemo}
          ...
          {sections.map((sec, sIdx) => (
            <div key={sIdx} className="bg-white border border-orange-100 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                />
                <button onClick={() => removeSection(sIdx)} className="px-3 py-2 rounded-lg bg-red-500 text-white hover:bg-red-600">
                  Xoá chương
                </button>
              </div>
              ...
              <div className="space-y-2">
                {(sec.lessons || []).map((ls, lIdx) => {
                  const type = ls.type || "video";

                  return (
                    <div key={lIdx} className="border-t pt-3 first:border-t-0">
                      <div className="grid md:grid-cols-4 gap-2">
                        ...
                        <select
                          ...
                          className="border rounded-lg px-3 py-2 border-orange-200 focus:ring-2 focus:ring-orange-300 outline-none bg-white"
                        >
                      ...
                      <div className="mt-2">
                        {type === "video" ? (
                          ...
                          <textarea
                            ...
                            onChange={(e) => updateLesson(sIdx, lIdx, "textContent", e.target.value)}
                            className="w-full border rounded-lg px-3 py-2 border-orange-200 focus:ring-2 focus:ring-orange-300 outline-none min-h-[120px]"
                            placeholder="Nội dung bài đọc (Text)..."
                          />
                        )}
                      </div>
                    </div>
