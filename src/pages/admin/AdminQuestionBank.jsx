...
import examApi from "../../api/examApi"; // dùng để gọi AI generate-questions

const API_BASE = "http://localhost:5000/api";

// 🔍 Đoán "môn học" từ tên khoá học để gắn tag / hướng AI
const guessSubjectFromCourseTitle = (title = "") => {
  const t = title.toLowerCase();

  if (t.includes("tiếng anh") || t.includes("ielts") || t.includes("toeic")) {
    return "english";
  }
  ...
  if (t.includes("toán") || t.includes("math")) {
    return "math";
  }

  if (t.includes("logic") || t.includes("tư duy")) {
    return "logic";
  }
...
export default function AdminQuestionBank() {
  ...
  const [aiSubject, setAiSubject] = useState("other"); // không show ra UI, chỉ dùng nội bộ
  ...
  const [savingAiIndex, setSavingAiIndex] = useState(null); // câu AI đang lưu từng câu
  const [savingAllAi, setSavingAllAi] = useState(false); // lưu tất cả
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        // Mặc định: admin nhìn thấy tất cả khoá học
        let url = `${API_BASE}/admin/courses`;
        let params = { page: 1, limit: 1000 };

        // Nếu là giảng viên: ưu tiên gọi API lấy khoá học của riêng giảng viên
        if (role === "teacher") {
        ...
        try {
          ...
          // Nếu là teacher và route /teacher/my-courses chưa có → fallback về admin/courses
          if (role === "teacher") {
            console.warn(
              "[QuestionBank] /teacher/my-courses lỗi, fallback về /admin/courses",
              err?.response?.status
            );
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      ...
      try {
        const data = await examQuestionApi.admin.getFilters(course);
        setFilters({
  ...
  const handleCourseChange = (e) => {
    ...
    // 🔄 đổi khoá học thì clear kết quả AI cũ
    setAiQuestions([]);

    // 🔍 tự đoán môn học cho AI (dùng cho tag / prompt) nhưng không hiện ra UI
    const selectedCourse = courses.find((c) => c._id === value);
    if (selectedCourse) {
      const title = selectedCourse.title || selectedCourse.name || "";
      const subject = guessSubjectFromCourseTitle(title);
  ...
  const handleImport = async () => {
    if (!course) {
      alert("Vui lòng chọn khoá học trước khi import!");
      return;
    }
    if (!importFile) {
    ...
    try {
      ...
      const res = await axios.post(
        `${API_BASE}/admin/exam-questions/import`,
        formData,
        {
          headers: {
      ...
      alert(`✔ Import thành công ${res.data.count} câu hỏi`);
      setImportFile(null);
      await loadQuestions();
    } catch (err) {
  ...
  /* ================= XOÁ TOÀN BỘ CÂU HỎI CỦA KHOÁ ================= */
  const handleDeleteAll = async () => {
    if (!course) {
      alert("Vui lòng chọn khoá học trước.");
      return;
    }

    const firstConfirm = window.confirm(
      "Bạn chắc chắn muốn XOÁ TOÀN BỘ ngân hàng câu hỏi của khoá học này?"
    );
    if (!firstConfirm) return;

    const secondConfirm = window.confirm(
      "Hành động này KHÔNG THỂ hoàn tác. Nhấn OK để xoá vĩnh viễn TẤT CẢ câu hỏi."
    );
    ...
    try {
      ...
      // lấy toàn bộ câu hỏi (bỏ filter)
      const res = await examQuestionApi.admin.getQuestions({
        course,
        page: 1,
        limit: 5000,
      });
      ...
      if (!items.length) {
        alert("Khoá học này hiện không có câu hỏi nào.");
        return;
      }

      const promises = items.map((q) =>
        examQuestionApi.admin.deleteQuestion(q._id)
      );
      ...
      alert(
        "Có lỗi khi xoá hàng loạt. Một số câu có thể chưa bị xoá hết, hãy kiểm tra lại."
      );
    } finally {
  ...
  const handleGenerateByAI = async () => {
    if (!course) {
      alert("Vui lòng chọn khoá học trước khi dùng AI sinh câu hỏi.");
      return;
    }
    ...
    try {
      ...
      const payload = {
        subject: aiSubject, // không hiện ra UI nhưng vẫn gửi cho AI
        topic: aiTopic,
        level: aiLevel,
      ...
      // ⚠️ cần có hàm generateByAI trong examApi.questionBank
      const res = await examApi.questionBank.generateByAI(payload);

      if (!res.ok) {
        setAiError(res.error || "AI không sinh được câu hỏi, thử lại sau.");
        return;
      }
  ...
  const buildPayloadFromAIQuestion = (q) => {
    return {
      ...
      chapter: chapter || aiTopic || "",
      difficulty: aiLevel,
      tags: [aiSubject, ...(aiTopic ? [aiTopic] : [])].filter(Boolean),
      score: 1,
  ...
  const handleSaveAIQuestion = async (q, index) => {
    ...
    try {
      ...
      alert("Lưu câu hỏi AI thất bại. Kiểm tra log server.");
    } finally {
      setSavingAiIndex(null);
    }
  };

  /* ================= 💾 LƯU TẤT CẢ CÂU AI VÀO NGÂN HÀNG ================= */
  const handleSaveAllAIQuestions = async () => {
    ...
    if (!aiQuestions.length) {
      alert("Chưa có câu hỏi AI nào để lưu.");
      return;
    }
    ...
    try {
      ...
      await Promise.all(
        payloads.map((p) => examQuestionApi.admin.createQuestion(p))
      );

      await loadQuestions();
      alert(`✔ Đã lưu ${aiQuestions.length} câu hỏi AI vào ngân hàng.`);
    } catch (err) {
      console.error("Lỗi lưu tất cả câu hỏi AI:", err);
      alert("Lưu tất cả câu hỏi AI thất bại. Kiểm tra log server.");
    } finally {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Quản lý – Import – Tạo mới các câu hỏi cho khoá học.
          </p>
        </div>
      ...
      <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-4 mb-4 space-y-3">
        ...
        <div className="pt-3 border-t border-slate-200">
          ...
          <p className="text-[11px] text-slate-400 mt-1">
            File mẫu: content | type | options (a|b|c) | correct | chapter |
            difficulty | tags | score
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-4 mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          {!course && (
            <span className="text-[11px] text-red-500">
              Chọn khoá học trước khi dùng AI.
            </span>
          )}
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
          {/* Khoá học đang chọn (chỉ hiển thị, không sửa) */}
          <div>
            ...
            <input
              ...
              value={
                course
                  ? courseMap[course] || "Khoá học không xác định"
                  : "Chưa chọn khoá học"
              }
              disabled
            />
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="VD: Hàm số bậc hai, Mảng trong JavaScript, Thì hiện tại hoàn thành..."
              disabled={!course}
            />
          </div>
        ...
        {/* DANH SÁCH CÂU HỎI AI SINH RA */}
        {aiQuestions.length > 0 && (
          <div className="mt-4 border-t pt-3">
            <p className="text-xs text-slate-500 mb-2">
              AI đã sinh {aiQuestions.length} câu. Bạn có thể lưu trực tiếp vào
              ngân hàng hoặc chỉnh sửa lại trước khi dùng.
            </p>
      ...
      <div className="bg-white border border-slate-100 rounded-xl shadow-sm">
        <div className="border-b border-slate-100 px-4 py-2 flex items-center justify-between">
          <span className="text-sm text-slate-600">
            {course
              ? `Đang xem ${questions.length} câu hỏi`
          ...
          {course && (
            ...
            >
              {deletingAll
                ? "Đang xoá..."
                : "🗑 Xoá toàn bộ câu hỏi của khoá này"}
            </button>
          )}
        </div>
                  ...
                  <td className="px-4 py-2 max-w-xl">
                    ...
                    <div className="mt-1 flex flex-wrap gap-1">
                      {q.tags?.map((t) => (
                        <span
                          key={t}
                          className="px-2 py-[2px] rounded bg-orange-50 text-[11px] text-orange-600"
                        >
                          #{t}
                        </span>
                      ))}
