...
export default function StudentExamDo() {
  ...
  // 🎯 Kết quả phân tích AI (Skill Map / gợi ý học lại)
  const [aiResult, setAiResult] = useState(null);

  // ✨ Popup kết quả đẹp ở giữa màn hình
  ...
  // ===========================================================
  // 10s COUNTDOWN TRƯỚC KHI BẮT ĐẦU
  // ===========================================================
  useEffect(() => {
  ...
  const handleStart = async () => {
    ...
    try {
      const data = await examApi.student.start(id); // GET /api/exams/:id/start
  ...
  // ===========================================================
  // COUNTDOWN THEO timeLimit (THỜI GIAN LÀM BÀI)
  // ===========================================================
  useEffect(() => {
  ...
  // ========== UPLOAD FILE CHO CÂU HỎI TỰ LUẬN / BÀI TẬP ==========
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await examApi.student.submit(id); // { message, attempt }
      ...
      // 🎯 ====== GỌI AI PHÂN TÍCH SAU KHI NỘP BÀI ======
      try {
        ...
        const courseId =
          exam?.course?._id || exam?.courseId || exam?.course || null;
      ...
      if (err?.response?.data?.message === "Đã quá hạn nộp bài") {
        setIsOverDue(true);
      }
  ...
  // ===========================================================
  // UI: ĐÃ NỘP – HIỂN THỊ KẾT QUẢ + PHÂN TÍCH AI
  // ===========================================================
  if (phase === "submitted") {
    const att = submittedAttempt;

    const totalQuestions = Array.isArray(att?.answers)
      ? att.answers.length
      : 0;
    ...
    // 👉 Nút chuyển sang trang bản đồ kỹ năng chi tiết
    const handleViewSkillMap = () => {
      ...
      if (!courseId || !attInner?._id) {
        window.alert(
          "Thiếu dữ liệu khoá học hoặc bài làm, không mở được bản đồ kỹ năng."
        );
        return;
      }
      ...
      <>
        <div className="max-w-xl mx-auto p-6">
          <div className="bg-white rounded-xl border shadow-soft p-6 text-center">
            ...
            {canViewScore ? (
              <>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Các câu trắc nghiệm đã được chấm tự động. Những câu tự luận /
                  nộp file (nếu có) sẽ được giáo viên chấm thêm, điểm có thể thay
                  đổi.
                </p>
              </>
            ) : (
            ...
            {/* Box AI phân tích Skill Map / gợi ý ôn tập */}
            {renderAiBox()}
                ...
                <>
                  <p className="text-sm text-slate-600 mb-3">
                    ...
                    <span className="font-semibold text-emerald-600">
                      {resultPopup.correctQuestions}/
                      {resultPopup.totalQuestions}
                    </span>{" "}
                    câu.
                  </p>
                  ...
                  <p className="text-xs text-slate-500 mb-4">
                    Một số câu tự luận hoặc nộp file có thể được giáo viên chấm
                    thêm sau.
                  </p>
                </>
              ) : (
      ...
      <div className="bg-white rounded-xl border shadow-soft p-6">
        ...
        {!isOverDue &&
          ...
          dueRemainingSeconds <= 300 && (
            <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 text-yellow-800 rounded text-sm">
              ⏰ Chỉ còn chưa đầy 5 phút trước hạn nộp bài. Hãy rà soát và nộp
              bài sớm để tránh quá hạn.
            </div>
          )}

        {/* HẾT THỜI GIAN LÀM BÀI (timeLimit) */}
        {isTimeUp && (
          <div className="p-3 bg-red-50 text-red-600 rounded mb-4 text-sm">
            Thời gian làm bài đã hết. Nếu bạn chưa kịp nộp, hệ thống có thể tự
            chấm dựa trên câu trả lời đã lưu.
          </div>
        )}
              ...
              <div key={q._id} className="p-4 border rounded-lg bg-orange-50">
                ...
                {type === "multiple_choice" &&
                  optionSnapshots.map((op) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        disabled={isOverDue || isTimeUp}
                        checked={selectedIds.includes(
                          String(op._id || op.optionId)
                        )}
                        onChange={() =>
                          handleOptionChange(q, op._id || op.optionId, true)
                        }
                      />
                      <span>{op.text}</span>
                    </label>
                ...
                {(type === "short_answer" || type === "essay") && (
                  <>
                    ...
                    {/* Nếu là đề bài tập -> cho phép nộp file kèm theo */}
                    {exam.type === "assignment" && (
                      <div className="mt-2">
        ...
        <div className="mt-6 text-right">
          <button
            ...
            disabled={submitting || isTimeUp || isOverDue}
