...
const resolveCourseImage = (courseWrapper) => {
  // nhiều API trả dạng { course: {...realCourse}, progress: ... }
  const course = courseWrapper.course || courseWrapper;

  // 1. Ưu tiên các key hay dùng
  let img =
    course.thumbnail ||
    course.image ||
    course.courseImage ||
  ...
  // 2. Nếu vẫn chưa có -> quét toàn bộ object tìm string có vẻ là ảnh
        ...
        {courses.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-soft p-8 text-center border border-orange-100">
            <p className="text-muted mb-3">
              Bạn chưa đăng ký khoá học nào. Bắt đầu hành trình học tập ngay
              hôm nay nhé!
            </p>
            <Link
              to="/courses"
                  ...
                  <div className="md:w-1/3 relative">
                    <Link to={detailLink} className="block h-full">
                      <img
                        src={
                          img ||
                          "https://placehold.co/400x240?text=Course"
                        }
                        alt={rawCourse.title}
